...
Teams %>% filter(yearID %in% (1961:2001)) %>% summarize(r = cor(R/G, AB/G))

# correlation coefficient between win rate (number of wins per game) and number of errors per game
Teams %>% filter(yearID %in% (1961:2001)) %>% summarize(r = cor(W/G, E/G))

# orrelation coefficient between doubles (X2B) per game and triples (X3B) per game
Teams %>% filter(yearID %in% (1961:2001)) %>% summarize(r = cor(X2B/G, X3B/G))
...
# which is the best prediction for y given that we know the value of x 
# is given by the regression line
...
# Variance Explained
######################################
# When two variables follow a bivariate normal distribution, 
# the variation explained can be calculated as  rho^2ª100
...
mean(scale(female_heights$mother)*scale(female_heights$daughter))
...
####################################
# Although interpretation is not straight-forward, 
# it is also useful to know that the LSE can be strongly correlated, 
...
dat <- Teams %>% 
  filter(yearID %in% 1961:2001) %>%
  mutate(R_per_games = R/G, BB_per_games = BB/G, HR_per_games = HR/G)
  
lm(R_per_games ~ BB_per_games + HR_per_games, data = dat)
...
bat_mean <- Batting %>% filter(yearID %in% 1999:2001) %>%
  mutate(pa = AB + BB) %>%
  filter(pa >= 100) %>%
  mutate(pa = AB + BB, singles = (H - X2B - X3B - HR)/pa, bb = BB/pa) %>%
  group_by(playerID) %>%
...
bat_mean %>% filter(mean_singles > 0.2) %>% count()

bat_mean %>% filter(mean_bb > 0.2) %>% count()

dat <- inner_join(bat_02, bat_mean, "playerID")
...
#                        DO                     #
#################################################
# 1. The do() function serves as a bridge between R functions, such as lm(), and the tidyverse.
# 2. We have to specify a column when using the do() function, otherwise we will get an error.
# 3. If the data frame being returned has more than one row, the rows will be concatenated appropriately.
